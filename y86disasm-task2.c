...
int main ( int argc, char** argv )
{
  ...
  while( fgets(buffer, 15, pFile ) )
  {
      ...
      //Any undisassembled opcode will display as "TODO: undisaasembled opcode and operands"
            unsigned char regA, regB, reg;
            reg = instruction[1];
            
            regA = reg >> 4;
            regB = reg & 0x0f;
            //printf("%X, %X, %X", reg, regA, regB);
            //regB = reg % 10;
            //regA = reg;
            //while(regA >= 10){
              //regA = regA / 10;
            //}
            //printf("%d\n",reg);
            //printf("%u",instruction[0]);
            //printf("%u",instruction[1]);
            //printf("%u",instruction[2]);
            //printf("%u",instruction[3]);
            //printf("%u",instruction[4]);
